...
export function BoardsPage(): JSX.Element {
  ...
  const onDeleteBoard = async (boardId: string): Promise<void> => {
    const confirmed = window.confirm("Delete this board and all its lists?");
    if (!confirmed) {
      return;
    }
            ...
            <Card key={board.id} className="overflow-hidden">
              ...
              <CardContent className="flex items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => void onDeleteBoard(board.id)}>
                    Delete
                  </Button>
                </div>
              </CardContent>
