...
const envSchema = z.object({
  ...
  REGISTRATION_HONEYPOT_ENABLED: zBoolean.default(true),
  ...
  UNASSIGNED_ACCOUNT_CLEANUP_MODE: z.enum(["disabled", "report", "delete"]).default("disabled"),
  UNASSIGNED_ACCOUNT_RETENTION_HOURS: z.coerce.number().int().min(1).max(24 * 365).default(48),
  UNASSIGNED_ACCOUNT_CLEANUP_BATCH_SIZE: z.coerce.number().int().min(1).max(100).default(25),
  UNASSIGNED_ACCOUNT_REQUIRE_RECENT_BACKUP: zBoolean.default(true),
  UNASSIGNED_ACCOUNT_MAX_BACKUP_AGE_HOURS: z.coerce.number().int().min(1).max(24 * 30).default(26),
  UNASSIGNED_ACCOUNT_PROTECTED_EMAILS: z.string().default(""),
  UNASSIGNED_ACCOUNT_PROTECTED_USER_IDS: z.string().default(""),
